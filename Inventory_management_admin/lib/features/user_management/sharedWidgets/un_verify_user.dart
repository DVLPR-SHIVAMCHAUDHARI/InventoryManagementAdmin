...
class UnVerifyUser extends StatelessWidget {
  UnVerifyUser({super.key});
  ...
  Widget build(BuildContext context) {
    return BlocListener<UserManagementBloc, UserManagementState>(
      listener: (context, state) {
        if (state is UnVerifyUserSuccess) {
          snackbar(
            context,
            color: Colors.green,
        ...
        if (state is UnVerifyUserFailure) {
          snackbar(
            context,
            color: Colors.red,
            message: state.error,
                    ...
                    onPressed: () {
                      if (_formKey.currentState!.validate()) {
                        ...
                        context.read<UserManagementBloc>().add(
                          UnVerifyUserEvent(userId),
                        );
                      }
                    },
                    child: state is UnVerifyUserLoading
                        ? SizedBox(
                            height: 18,
                            width: 18,
