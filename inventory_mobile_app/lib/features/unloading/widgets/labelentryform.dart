...
class LabelEntryForm extends StatefulWidget {
  const LabelEntryForm({super.key});

  @override
  State<LabelEntryForm> createState() => _LabelEntryFormState();
}

class _LabelEntryFormState extends State<LabelEntryForm> {
  ...
  void initState() {
    super.initState();
    context.read<MasterBloc>().add(GetLabelListEvent());
  }
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<MasterBloc, MasterState>(
      builder: (context, masterState) {
        if (masterState is GetLabelListLoading) {
          return const Center(child: CircularProgressIndicator());
        }

        if (masterState is GetLabelListFailure) {
          return const Center(child: Text("Failed to load Labels"));
        }

        final LabelList = masterState is GetLabelListSuccess
            ? masterState.labels
            : [];
                      ...
                      return DropdownButtonFormField2<int>(
                        ...
                        items: LabelList.map((Label) {
                          return DropdownMenuItem<int>(
                            value: Label["id"],
                            child: Text(
                              "${Label["size"]}Inches - ${Label["party_name"]}",
                        ...
                        listener: (context, state) {
                          if (state.isSuccess) {
                            snackbar(
                              context,
                              color: Colors.green,
                              message: "Label entry successful",
                              title: "Success",
                            );
  ...
  void _submit(UnloadingState state) {
    ...
    if (state.labelId == null) {
      snackbar(context, message: "Please select Label");
      return;
    }
